...
const blockedProxy = new Map();

const TIMES_TO_MAKE_REQUEST = 20;
const getSpaceData = async (spaceId: string) => {
  try {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      randomProxy = response.data;

      const blockedProxies = blockedProxy.get(spaceId) || [];
      if (blockedProxies.includes(randomProxy)) continue;
      break;
    }

    const username = randomProxy.match('(?<=://)[^:@]+');
    const password = randomProxy.match('/:(.*?)@/');
    const host = randomProxy.match('///(.*?):/');
    const port = randomProxy.match('/:(d+)$/');
