...
async function main() {
  ...
  const twitterSpaceUserTypes = await prisma.twitterSpaceUserType.findMany({
    where: {
      type: {
  ...
  if (twitterSpaceUserTypes.length < 3) {
    await prisma.twitterSpaceUserType.deleteMany();
    await prisma.twitterSpaceUserType.createMany({
      data: [{ type: 'CO-HOST' }, { type: 'SPEAKER' }, { type: 'LISTENER' }],
      skipDuplicates: true,
    });
  }
}
