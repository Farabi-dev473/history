...
const randomNumberController = (request: Request, response: Response) => {
  try {
    // converting query param start & end to number
    let start: nullOrNumber = parseInt(request.query.start as string);
    let end: nullOrNumber = parseInt(request.query.end as string);
