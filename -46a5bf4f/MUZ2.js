...
function getTxType(transaction) {
  ...
  const isSale =
    (instructions?.has("createAccount") &&
      ...
      logMessages?.includes("Program log: Instruction: BuyNft") ||
...
const getBuyerAndSeller = (tx, programId) =>{ 
  ...
  if(programId === "M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K" || programId === "A7p8451ktDCHq5yYaHczeLMYsjRsAkzc3hCXcSrwYHU7") {
    const {meta: innerInstructions} = tx
    const {transaction: {message: accountKeys}} = tx
    const buyer = accountKeys[0].pubkey
    const {instructions} = innerInstructions[innerInstructions.length - 1]
    const seller = instructions[instructions.length - 1]?.parsed?.info?.destination
  ...
  if(programId === "hadeK9DLv9eA7ya5KCTqSvSvRZeJC3JgD5a9Y3CNbvu") {
    const {meta: innerInstructions} = tx
    const buyer = innerInstructions[0]?.instructions[0]?.parsed?.info?.source
  ...
  if(programId === "CJsLwbP1iu5DuUikHEJnLfANgKy6stB2uFgvBBHoyxwz") {
    const {meta: innerInstructions} = tx
    const buyer = innerInstructions[0]?.instructions[1]?.parsed?.info?.source
    const {instructions} = innerInstructions[innerInstructions.length - 1]
    const seller = instructions[instructions.length - 1]?.parsed?.info?.destination
  ...
  if(programId === "5SKmrbAxnHV2sgqyDXkGrLrokZYtWWVEEk5Soed7VLVN" || programId === "hausS13jsjafwWwGqZTUQRmWyvyxn9EQpqMwV1PBBmk") {
      return {
        buyer: accounts[0],
        seller: accounts[1]
      }
...
async function main() {
  ...
  transactions.forEach((tx, i) => {
    ...
    // console.log({txType, i, txId: transactionIds[i]})
    if (programParser?.schemaFields) {
      ...
      if(buyerPrice) {
        ...
        console.log({i, txId: transactionIds[i], txType, slug: programData.slug, buyerPrice, amount})
      } 
    }  
  })
