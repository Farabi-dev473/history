...
const HoldingRequirment = ({id, numOfNFT, collection, trait, handleOnChange, handleOnClick}) => {
  id = id.substring(0, 5)
  ...
  <div className="bg-gray-200 p-4 rounded-md">
    <div className="flex">
      ...
      <div>
      <button type="button"  id={id} className="text-gray-500   hover:text-gray-700 rounded-sm w-6 h-6" onClick={handleOnClick} >
<svg id={id} 
...
const Page = () => {
  ...
  const [holdingRequirements, setHoldingRequirements] = useState([])
  ...
  const handleOnChange = (event) => {
    if(event.target.name === 'twitter-url') {
    ...
    else if(event.target.name === 'nft' || event.target.name === 'collection' || event.target.name === 'trait' || typeof event.target.name === 'string') {
     console.log(event.target.name, event.target.value)
      holdingRequirements.forEach((requirement, index) => {
        ...
        if(requirement.id === event.target.key) {
