...
const handleSpace = (spaceId: string) => {
    ...
    }, parseInt(process.env.INTERVAL_TIME as string) * 6000s0)
}
...
const spaceController = async (req: Request, res: Response) => {
    ...
    spaceIds.forEach(async (spaceId: string) => {
        await prisma.members.deleteMany({where: {time: {gte: 0}}})
        const spaceData = await getSpaceData(spaceId)
        ...
        res.status(200).send({msg: "Scheduler has been started successfully"})
        spaceQueue.add(() => {
            handleSpace(spaceId)
        })
    })
