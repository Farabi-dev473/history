...
export class AccountLinkingResolver {
  ...
  async linkTwitter(
    ...
    if (cacheTokenResponse) {
      ...
      try {
        ...
        throw new ApolloError(error?.statusText || 'Unable to link Twitter due to invalid authorization code');
      }
    }
    const {
    ...
    if (
      ...
      throw new ApolloError("Unable to link Twitter. Missing tokens.", "TWITTER_AUTH_MISSING_TOKEN");
    }
    ...
    if (!twitterUser || !twitterUser.id) {
      throw new ApolloError("Unable to authenticate with Twitter");
    }
  ...
  async loginTwitter(
    ...
    if (
      ...
      throw new ApolloError(
        'Unable to authenticate with Twitter. Missing token.',
        'TWITTER_AUTH_MISSING_TOKEN'
      );
    }
    ...
    if (!twitterUser || !twitterUser.id)
      throw new ApolloError(
        'Unable to authenticate with Twitter. Missing user data.',
        'TWITTER_AUTH_MISSING_USER_DATA'
      );
  ...
  async loginDiscord(
    ...
    if (
      ...
      throw new ApolloError(
        'Unable to authenticate with Discord. Missing token.',
        'DISCORD_AUTH_MISSING_TOKEN'
      );
    }
    ...
    if (!discordUser || !discordUser.id)
      throw new ApolloError(
        'Unable to authenticate with Discord. Missing user data.',
        'TWITTER_AUTH_MISSING_USER_DATA'
      );
    let userName = null;
