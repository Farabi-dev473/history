...
import getInstructionData from './packages/helpers/getInstructionData.js';
...
async function main() {
  ...
  transactionIds.forEach((txId, i) => {
    ...
    if(data.lamports) {
       ...
       const {slug} = getInstructionData(tx, txType)
       console.log({i, txId, txType, slug, lamports, price, buyer, seller, owner, mintAddress})
       return
    }

    console.log({i, txId, txType, err: data.error})
  })
