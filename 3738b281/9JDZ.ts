...
class BaseProgram {
  ...
  #getTransactionPrice() {
    ...
    if (
      ...
      if (this.mintAddress && programParser?.schemaFields) {
        console.log(programParser.schemaFields)
  ...
  #handleTransfer() {
    if(this.isProgram("digital-eyes")){
        this.seller = this.transaction?.meta?.innerInstructions?.[0]?.instructions[0]?.parsed?.info?.multisigAuthority
...
const txIds = ["5g4pDLjDggzNKKhPdwc9C1hyjVokbdz18EGPNwGGJXSHvkrSaDFUTnEBu5gWDFQ4eBaVGfDy2RWxjxeMUoy9ma5M"]
