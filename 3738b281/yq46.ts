...
class BaseProgram {
  ...
  #handleTransfer() {
    if(this.isProgram("digital-eyes")){
        this.seller = 
        this.buyer = this.transaction?.transaction?.message?.accountKeys[0]?.pubkey;
    }
    ...
    if(!this.seller){
      this.seller = this.transaction?.meta?.innerInstructions?.[0]?.instructions[0]?.parsed?.info.source
    }

    if(!this.seller) {
      this.seller = this.transaction?.transaction?.message?.accountKeys[0]?.pubkey;
    }
...
const txIds = ["5oNaVsvK41L9UfimWnyPrUFqgKM1wtZxNRatXuJhoR9Nyz31UQbsFBV4XZb38PVR7pwUmcEhfLpGgvnBFG93Rokr"]

txIds.forEach( async (txId) => {
     const tx = await getTransaction(txId)
     console.log(singleTransactionParser(tx))
    //  fs.writeFileSync(process.cwd() + '/data.json', jsonFormat(tx))
     const startTime = new Date().getTime()
     const delayTime = 20000
     while(startTime + delayTime > new Date().getTime()){}
});
