    ...
    this.INTERVAL_ID = setInterval(async () => {
      try {
        ...
        if (state === 'Ended' || state === 'TimedOut') {
          ...
          const handleFollowHostRequiredConstraints = async () => {
            ...
            holdingReqEligableUsers.forEach(async (user) => {
              console.log('data');
              ...
              console.log('data2');
              if (response.data) {
                usersFollowsHost.push(user);
              }
            });
          };

          if (followHostRequired) {
            await handleFollowHostRequiredConstraints();
            console.log(usersFollowsHost);
