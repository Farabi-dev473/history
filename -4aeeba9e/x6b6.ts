...
class TwitterSpaceUtils {
  ...
  private static INTERVAL_TIME_IN_MS = parseInt(
    process.env.INTERVAL_TIME_IN_MS || '30000'
  );
    ...
    this.INTERVAL_ID = setInterval(async () => {
      try {
          ...
          await ctx.prisma.twitterSpaceUserDetails.createMany({
            data: newMembers.map((newMember) => {
              return {
                ...
                userTypeInSpace: liveMembers.find((member) => {
        ...
        if (state === 'Ended' || state === 'TimedOut') {
          ...
          console.log('Debug 1');
          // filter users who has logged in with twitter
          ...
          const holdingReqEligableUsers = twitterLoggedInUsers.filter(
            async (user) => {
              let isEligable = true;
          ...
          if (followHostRequired) {
            ...
            const responses = await Promise.all(
              holdingReqEligableUsers.map((user) => {
                return axios.get(
            ...
            responses.forEach((response, index) => {
              if (response.data) {
                usersFollowsHost.push(holdingReqEligableUsers[index]);
              }
            });
          ...
          while (i < numberOfWinners) {
            ...
            if (winners.length === allConstraintsFilledUsers.length) {
              break;
            }else(!winners.includes(allConstraintsFilledUsers[winnerIndex])) {
  ...
  clearInterval() {
    console.log('Clearning Interval, Interval id  - ', this.INTERVAL_ID);
