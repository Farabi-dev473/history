...
async function twitterUserDetails(accessToken: string) {
  ...
  return getFetch('https://api.twitter.com/2/users/me', {
    headers: { authorization: `Bearer ${accessToken}` },
  });
}
...
async function exchangeTwitterCode(code: string, isLogin: boolean) {
  ...
  )}&grant_type=authorization_code&code_verifier=challenge&code=${code}&redirect_uri=${encodeURIComponent(
    `${redirect_uri}`
  )}`;

  console.log(body, redirect_uri);

  return getFetch('https://api.twitter.com/2/oauth2/token', {
    method: 'POST',
    body,
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
