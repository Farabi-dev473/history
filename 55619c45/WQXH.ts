...
async function twitterUserDetails(accessToken: string) {
  ...
  const profile = await fetch(
    `https://api.twitter.com/1.1/users/show.json?user_id=${twitterUserDetails.id}`,
    {
      method: 'GET',
      headers: {
        authorization: `Bearer AAAAAAAAAAAAAAAAAAAAAJXpnAEAAAAAM%2BskFDZiONwwZlLBv8QAyOfmPCI%3DjXHPph2GGHABnnc7TUoKyYs7nKkvXgKoAzs74u1PuDY3aEd7mM`,
      },
    }
  );

  return { ...(await response.json())?.data, ...(await profile.json()) };
}
