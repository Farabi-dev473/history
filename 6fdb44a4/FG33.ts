    ...
    try {
      ...
      if (spaceState === 'Running') {
        ...
        if (!isSpaceStarted) {
          ...
          let isSeeded = false;
          let twitterUserTypes: Prisma.TwitterSpaceUserTypeCreateInput[];
          ...
          userTypes.forEach((type) => {
            eligableUserTypes.push(
              twitterUserTypes.find(
                (twitterUser) => twitterUser.type === type.type
              )
            );
          });

          console.log('ELIGABLE USEr');
          // const eligableUsers = await ctx.prisma.eligableUserType.createMany({
