import { data } from '@moonly/staking';
    ...
    try {
      ...
      if (state === 'Running') {
        ...
        if (eligableUserTypes.includes('CO-HOST')) {
          ...
          spaceData.data.audioSpace.participants.admins.forEach((user) => {
            coHosts.push({
              twitterSpaceUserId: spaceId,
        ...
        if (eligableUserTypes.includes('SPEAKER')) {
          spaceData.data.audioSpace.participants.speakers.forEach((user) => {
            speakers.push({
              twitterSpaceUserId: spaceId,
              username: user.twitter_screen_name,
              display_name: user.twitter_displayname,
        ...
        if (eligableUserTypes.includes('LISTENER')) {
          spaceData.data.audioSpace.participants.listeners.forEach((user) => {
            listeners.push({
              twitterSpaceUserId: spaceId,
              username: user.twitter_screen_name,
              display_name: user.twitter_displayname,
