...
import { GiveawayInput, GiveawayResult, Role } from '../schema/TwitterSpace';
...
export class TwitterSpaceGiveawayResolver {
  @Query(() => GiveawayResult, { nullable: false })
  async showGiveawayWinners(
    ...
    try {
      ...
      if (spaceId.length != 13) {
        throw new ApolloError(
          'The space id you provided is invalid',
          'INVALID_INPUT'
        );
      }
      ...
      if (!twitterSpace?.id) {
        throw new ApolloError('This space was not monitored', 'NOT_FOUND');
      }

      if (twitterSpace.failedToListen) {
        throw new ApolloError('Failed to listen this space', 'TASK_FAILED');
      }
      ...
      const giveawayResult = {
        space: winners[0]?.TwitterSpace,
        winners: winners.map((winner) => {
          return {
            username: winner.TwitterSpaceUser?.username,
            avatarUrl: winner.TwitterSpaceUser?.avatarUrl,
          };
        }),
      };

      if (giveawayResult.winners.length === 0) {
        throw new ApolloError(
          'The space may be still running or no winners where eligable for winning the giveaway'
        );
      }
  ...
  async initGiveaway(
    @Arg('giveawayDetails') giveawayDetails: GiveawayInput,
    @Ctx() ctx: GQContext
  ) {
    try {
      const {
