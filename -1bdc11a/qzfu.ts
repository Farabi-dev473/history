...
import { HoldingRequirementInput, Role, Winner } from '../schema/TwitterSpace';
...
export class TwitterSpaceGiveawayResolver {
  ...
  async initGiveaway(
    @Arg('spaceId') spaceId: string,
    @Arg('minTimeInSec') minTimeInSec: number,
    @Arg('eligableUserTypes', () => [Role]) eligableUserTypes: Role[],
    @Arg('followHostRequired') followHostRequired: boolean,
    @Arg('holdingRequirements', () => [HoldingRequirementInput])
    holdingRequirements: HoldingRequirementInput[],
    @Arg('prizeName') prizeName: string,
    @Arg('numberOfWinners') numberOfWinners: number,
    @Ctx() ctx: GQContext
  ) {
    try {
      ...
      if (state === 'Running' || state === 'NotStarted') {
        ...
        let newHoldingRequirements: any = [];
        newHoldingRequirements =
      ...
      // if space is shceduled then set scheduler so that continiously listen the space & update data
      if (state === 'NotStarted') {
