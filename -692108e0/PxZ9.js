...
const getBuyerAndSeller = (tx, programId) =>{ 
   ...
   let seller 
    ...
    if(programId === "mmm3XBJg5gk8XJxEKBvdgptZz6SgK4tXvn36sodowMc") {
      ...
      seller = instructions[0]?.parsed?.info?.wallet
    ...
    if(programId === "TSWAPaqyCSx2KABk68Shruf4rp7CxcNi8hAsbdwmHbN") {
      ...
      const {instructions} = innerInstructions[0]
      seller = instructions[instructions.length - 1]?.parsed?.info?.destination
      buyer = instructions[0]?.parsed?.info?.wallet
  
      return {buyer, seller}
    }
  
  
    if(programId === "hadeK9DLv9eA7ya5KCTqSvSvRZeJC3JgD5a9Y3CNbvu" || programId === "5SKmrbAxnHV2sgqyDXkGrLrokZYtWWVEEk5Soed7VLVN" || programId === "hausS13jsjafwWwGqZTUQRmWyvyxn9EQpqMwV1PBBmk" || programId === "6U2LkBQ6Bqd1VFt7H76343vpSwS5Tb1rNyXSNnjkf9VL") {
      const accounts = getAccounts(tx)
      seller  = accounts[1]
      return {
        buyer,
