...
class Test {
  ...
  const result = [250000000000, 40000000000, 17000000000, 970000000, 17900000000, 15000000000, 52500000000, 100000000000, 5690000000, 15150000000, 300000000]
...
async function main() {
  ...
  transactions.forEach((tx, i) => {
    ...
    if (programParser?.schemaFields) {
      ...
      const keys = Object.keys(programs[txData.programId]["actions"])
      for(let j = 0; j < keys.length; j++) {
        const {schemaFields} = programs[txData.programId]["actions"][keys[j]]
        const TestSchema = new Map([
          ...
          try{
            const data = deserializeUnchecked(TestSchema, Test, buffer).buyerPrice + ''
          if (result.includes(parseInt(data))) {
            console.log({buyerPrice: data, i, key: keys[j]})
            break
          }
          }catch(err){}
      }
