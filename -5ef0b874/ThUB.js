...
const results = [250000000000, 40000000000, 17000000000, 970000000, 17900000000, 15000000000, 52500000000, 100000000000, 5690000000, 15150000000, 300000000]
    ...
    if (programParser?.schemaFields) {
      ...
      const keys = Object.keys(programs[txData.programId]["actions"])
        for(let i = 0; i < keys.length; i++) {
          const schemaFields = programs[txData.programId]["actions"][keys[i]]["schemaFields"]
        ...
        try{
          ...
          if (results.includes(data)) console.log({buyerPrice: hex, i})
