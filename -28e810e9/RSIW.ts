...
const INTERVAL_TIME = parseInt(
  process.env.REQUEST_INTERVAL_TIME_IN_MINUTE || '1000'
);
  ...
  async initGiveaway2(
    ...
    try {
      ...
      if (state === 'Ended') {
        ...
        const eligableUserWallets = [
          ...twitterLoggedInUsers
            .filter((user) => user.walletAddress != null)
            .map((user) => user.walletAddress),
          ...loggedInUsers.map((user) => user.address),
        ];

        let tokens: string[] = [];
        eligableUserWallets.forEach(async (address) =>
          tokens.push(
                  ...
                  AND: [
                    {
                      name: {
                        in: holdingRequirements.map(
                          (requirement) => requirement.collectionName
                        ),
                      },
                    },
                    {
        ...
        const users = await ctx.prisma.nFTItem.findMany({
          where: {
        ...
        holdingRequirements.forEach((requirement) => {
          if (selectedUserIds.length !== 0) {
            selectedUserIds.forEach((id) => {
              ...
              if (nftItems.length >= requirement.numberOfNft) {
                selectedUserIds.push(id);
              }
            });
          } else {
            users.forEach((user) => {
              const nfts = await ctx.prisma.nFTItem.findMany({
                where: {
                  userId: user.id,
                },
              });

              if (nfts.length >= requirement.numberOfNft) {
                selectedUserIds.push(user.userId as string);
              }
            });
          }
        });

        const giveawayEligableUsers = await ctx.prisma.user.findMany({
          where: {
            id: {
              in: selectedUserIds,
            },
        ...
        for (let i = 0; i < numberOfWinners; i++) {
          const index = Math.floor(
            Math.random() * giveawayEligableUsers.length
          );
          winners.push(giveawayEligableUsers[index]);
        }
