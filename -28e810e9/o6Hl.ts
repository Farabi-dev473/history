  ...
  async initGiveaway2(
    ...
    try {
      ...
      if(state === 'Ended') {
        ...
        const minTimeEligableUsers = await ctx.prisma.twitterSpaceUser.findMany({
          where: {
            AND: [
              {
                twitterSpace_id: spaceId
              ...
              {
                username: {in: loggedInUsers.map((user) => user.twitterUserName)}
              }
              {
       ...
       const holdingRequirements = await ctx.prisma.holdingRequirement.findMany({
        where: {
          twitterSpace_id: spaceId
        }
       })
