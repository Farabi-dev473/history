...
const redirect_uri = 'http://localhost:4000/auth/twitter/callback';
...
function validateUrlSlug(text) {
  text = "dkjfkdjf4r4ur9348r329jfifjna@"
  return text.match(/^[a-zA-Z0-9\-_]+$/);
}
console.log(validateUrlSlug((await user.json()).username))
}
...
app.get('/auth/twitter', (req, res) => {
    res.redirect(`https://twitter.com/i/oauth2/authorize?response_type=code&client_id=cmM4dHdydXBlaW9PN0JWZ09rMko6MTpjaQ&redirect_uri=http://localhost:4000/auth/twitter/callback&scope=tweet.read%20users.read%20follows.read%20offline.access&state=state&code_challenge=challenge&code_challenge_method=plain`)
})
