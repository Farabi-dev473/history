...
function Collection({handleOnChange, id}) {
  ...
  const [filteredCollection, setfilteredCollection] = useState(collections)
      ...
      useEffect(() => {
          async function fetchCollection() {
           try{
            const response = await axios.get(`http://localhost:4411/api/v1/collections/search/?q=${query}`)
      ...
      useEffect(() => {
        handleOnChange(selectedCollection, id)
      }, [selectedCollection])

  return <div className='text-black'>
     <Combobox value={selectedCollection} onChange={setSelectedCollection} name='collection'>
      ...
      <div className="bg-gray-200 p-4 rounded-md">
        <div className="flex">
          ...
          <div className="flex-1 ml-4 mb-4">
            ...
            <Collection handleOnChange={handleOnChange} id = {dataId}/>
          </div>
...
const Page = () => {
  ...
  let [holdingRequirements, setHoldingRequirements] = useState([
    { id: uuidv4(), numOfNFT: 1, collectionName: 'Moonly', trait: 'Any' },
  ]);
