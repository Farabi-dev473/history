  ...
  const [filteredCollection, setfilteredCollection] = useState(collections)
      ...
      useEffect(() => {
          async function fetchCollection() {
           try{
            const response = await axios.get(`http://localhost:4411/api/v1/collections/search/?q=${query}&limit=5`)
      ...
      useEffect(() => {
        handleOnChange({target: {name: 'collection', value: selectedCollection}}, id)
      }, [selectedCollection])
