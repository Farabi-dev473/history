...
const parseListEventTx = (txId) => {
    ...
    console.log(txId, tx, txType, programId, accounts)
    ...
    if(programId === "TSWAPaqyCSx2KABk68Shruf4rp7CxcNi8hAsbdwmHbN") {
        const pdaAccountIndex = 4
        pdaAddress = accounts[pdaAccountIndex]

        pdaAddress = tx?.meta?.innerInstructions[0]?.instructions[2]?.parsed?.info?.newAccount
    ...
    if(programId === "A7p8451ktDCHq5yYaHczeLMYsjRsAkzc3hCXcSrwYHU7") {
...
console.log(parseListEventTx("3EvCqzuf8QTiFTYe2nW3MFcCJHY224KHJU9gS6uXQxMdqXXqFAoyCKr6hzsJ8HEVFnBUx7cA8iz3odeim4zQJdeb"))
