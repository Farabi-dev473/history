...
const messageCreateHandler =  async (message: Message | GraphQLReqType) => {
    ...
    try {
      ...
      if(quotes.length.toString() === '0') {
        ...
        return 'No quotes found. Use /ask command to generate using AI.'
      }

      // return a random quote from the api result
      const randomIndex = Math.floor(Math.random() * quotes.length)
      const {quoteText, quoteGenre, quoteAuthor, _id} = quotes[randomIndex]
      if(isGraphQLRequest) {
        return {
          author: quoteAuthor,
          category: quoteGenre,
          ...
          (message as Message).reply(`${quoteText} - ${authorName}`)
      }
