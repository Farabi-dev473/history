...
const messageCreateHandler =  async (message: Message) => {
    ...
    try {
      ...
      authorName = entities[0]?.toLowerCase()
      categoryName = entities[1]?.toLowerCase()
...
if(data && data?.quotes?.length !== 0) {
  ...
  // check if the same quote has already been sent before
  let quote = quotes[Math.floor(Math.random() * quotes.length)].quote;
  while (sentQuotes.has(quote)) {
