...
class BaseProgram {
    constructor(transaction) {
        ...
        this.buyer = transaction?.meta?.innerInstructions?.[0]?.instructions.find((e) => e?.parsed?.info?.source)?.parsed?.info?.source;
        this.amount =
...
_BaseProgram_instances = new WeakSet(), _BaseProgram_getInitialSearchData = function _BaseProgram_getInitialSearchData() {
    ...
    const instructions = new Set(this.transaction?.meta?.innerInstructions
        ?.map((inner) => inner?.instructions?.map((instruction) => instruction?.parsed?.type?.trim()))
        .flat());
    return {
        logMessages,
        instructions,
    ...
    const { instructions, logMessages } = __classPrivateFieldGet(this, _BaseProgram_instances, "m", _BaseProgram_getInitialSearchData).call(this);
    const isMint = instructions?.has('initializeMint') ||
        logMessages?.includes('Instruction: MintNft') ||
        logMessages?.includes('Instruction: InitializeMint');
    if (isMint)
        return 'Mint';
    const isList = ((logMessages?.includes('Instruction: Sell') ||
        ...
        logMessages?.includes('Program log: Instruction: SellNftToLiquidityPair') ||
        logMessages?.includes('Program log: Instruction: TokenList') ||
        logMessages?.includes('Program log: Instruction: DepositNft');
    if (isList)
        return 'List';
    const isSale = (instructions?.has('createAccount') &&
    ...
    if (isUpdateList)
        return 'UpdateList';
    const isDelist = logMessages?.includes('Instruction: Cancel') ||
    ...
    const accounts = new Set(this.transaction?.transaction?.message?.accountKeys?.map((e) => e.pubkey));
    ...
    const programParser = this.programData?.programData?.actions[this.transactionType];
    ...
    let buyerPrice = null;
    if (this.programData?.slug === 'hadeswap' &&
        this.transactionType === 'List') {
        buyerPrice =
            this.transaction?.meta?.innerInstructions[0]?.instructions?.reduce((pre, cur) => {
    ...
    else if (this.programData?.slug === 'exchange-art' &&
        this.transactionType === 'Sale') {
        buyerPrice =
            this.transaction?.meta?.innerInstructions[1]?.instructions?.reduce((pre, cur) => {
                if (cur?.parsed?.type === 'transfer') {
                    return (cur?.parsed?.info?.lamports || 0) + pre;
                }
                return pre;
        ...
        if (this.mintAddress && programParser?.schemaFields) {
            ...
            try {
                buyerPrice = deserializeUnchecked(schema, Primitive, Buffer.from(bs58.decode(txData.data))).buyerPrice.toNumber();
            }
            catch (e) {
                // console.log(e);
    ...
    return this.transaction?.meta?.postTokenBalances?.find((account) => account?.uiTokenAmount?.amount === '1')?.owner;
}, _BaseProgram_handleMint = function _BaseProgram_handleMint() {
    let instruction;
    const inner = this.transaction.meta.innerInstructions.find((innerIns) => innerIns.instructions.some((ins) => ins.parsed));
    instruction = inner?.instructions.find((ins) => ins?.parsed?.info?.authorityType === 'mintTokens');
    if (!instruction)
        instruction = this.transaction.transaction.message.instructions.find((ins) => ins?.parsed?.type === 'initializeMint');
    if (instruction) {
    ...
    if (this.isProgram('yawww')) {
        const mostCoinReceiver = this.transaction?.meta?.innerInstructions?.[0]?.instructions
            .filter((instruction) => instruction?.parsed?.type === 'transfer' &&
            instruction?.programId === '11111111111111111111111111111111')
            .sort((a, b) => b?.instruction?.parsed?.info?.lamports -
            a?.instruction?.parsed?.info?.lamports)?.[0];
    ...
    if (this.isProgram('hyperspace')) {
        const transferRaw = this.transaction?.meta?.innerInstructions?.[0]?.instructions?.[0]
            ?.parsed;
        if (transferRaw?.type === 'transfer' && transferRaw?.info?.lamports) {
            this.amount = transferRaw?.info?.lamports / 1000000000;
    ...
    if (this.isProgram('hyperspace') ||
        ...
        this.seller = this.transaction.meta.postTokenBalances.find((account) => account.uiTokenAmount.amount === '0')?.owner;
    }
    ...
    const amountRaw = this.logMessageJson
        ...
        ?.at(-1);
    if (amountRaw) {
        this.amount = amountRaw && JSON.parse(amountRaw)?.price / 1000000000;
    }
    else {
    ...
    if (this.isProgram('solanart') ||
        ...
        this.seller = this.transaction.meta.postTokenBalances.find((e) => e?.uiTokenAmount?.amount === '0')?.owner;
    }
    ...
    if (!this.seller && this.isProgram('MagicEdenV2')) {
        this.seller = this.transaction?.transaction?.message?.instructions?.find((instruction) => instruction.programId ===
            'M2mx93ekt1fmXSVkTrUL9xVFHkmME8HTUi5Cyc5aF7K')?.accounts?.[0];
    }
    if (!this.seller)
        console.log(['Error seller on delist', this.transactionId]);
    this.buyer = null;
}, _BaseProgram_handleTransfer = function _BaseProgram_handleTransfer() {
    this.seller = this.transaction?.meta?.preTokenBalances?.find((account) => account?.uiTokenAmount?.amount === '1')?.owner;
    this.buyer = this.transaction?.meta?.postTokenBalances?.find((account) => account?.uiTokenAmount?.amount === '1')?.owner;
...
const txIds = Object.keys(JSON.parse(fs.readFileSync(process.cwd() + '/db.json', 'utf8')));

txIds.forEach( async (txId) => {
 const tx = await getTransaction(txId)
 const time = new Date()
 // write down js code which blocks execution for 1 second 
