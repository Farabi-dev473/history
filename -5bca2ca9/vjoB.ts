...
const spaceIds :any[] = ['1gqGvypgnokKB']
const concurrencyNumber = spaceIds.length
const spaceQueue = new PQueue({concurrency: 1})
    ...
    spaceIds.forEach(async (spaceId) => {
       await spaceQueue.add(() => {
        setInterval( async() => {
          ...
          if(fs.existsSync(`../spaces/$${spaceId}.json`)) {
            fs.appendFileSync(`../spaces/${spaceId}.json`, JSON.stringify(spaceMembersData))
          }else{
            fs.writeFileSync(`../spaces/${spaceId}.json`, JSON.stringify(spaceMembersData))
          }
        }, intervalTimeInMs)
       }) 
    })
